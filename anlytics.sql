...
select round(sum(total_amount)::numeric / 1000000 , 2) || 'M' as total_revenue FROM tbl_analytics; 

--- bcoz total_amount is double precision round(doble_precision,2) does not exist, hence we are
--- changing to numeric. however, you can still round but then format would be round(double_precision)


----- **Daily Trends:** Total revenue and average fare per trip over time.  


select to_date(left(tpep_dropoff_datetime,10), 'YYYY-MM-DD') as dropoff_date,
round(sum(total_amount)::numeric / 1000000 , 4) || 'M' as total_revenue,
round(avg(fare_amount)::numeric, 2) as average_fare
from tbl_analytics
group by 1

--- If you notice the output 0.002M revenu does not have drop_off_date which means drop off timestamp was not captured
---- or data might have been updated wrongly

select distinct to_date(left(tpep_dropoff_datetime,10), 'YYYY-MM-DD') from tbl_analytics
...
select min(fare_amount), max(fare_amount), round(avg(fare_amount)::numeric,2) from tbl_analytics
where fare_amount > 0;
...
count(*) as total_rows,
SUM(case when fare_amount < ( select round(avg(fare_amount)::numeric,2) FROM tbl_analytics ) then 1 end ) as low_fare,
SUM(case when fare_amount > ( select round(avg(fare_amount)::numeric,2) FROM tbl_analytics ) then 1 end ) as high_value_fare,
SUM(case when ( fare_amount > ( select round(avg(fare_amount)::numeric,2) FROM tbl_analytics )  
and 
fare_amount < ( select round(avg(fare_amount)::numeric,2) FROM tbl_analytics ) ) then 1 end )  as medium_fare,
SUM(case when fare_amount < 0 then 1 end ) as negative_fare,

COUNT(case when fare_amount < ( select round(avg(fare_amount)::numeric,2) FROM tbl_analytics ) then 1 end ) as low_fare2,
COUNT(case when fare_amount > ( select round(avg(fare_amount)::numeric,2) FROM tbl_analytics ) then 1 end ) as high_value_fare2,
COUNT(case when ( fare_amount > ( select round(avg(fare_amount)::numeric,2) FROM tbl_analytics )  
and 
fare_amount < ( select round(avg(fare_amount)::numeric,2) FROM tbl_analytics ) ) then 1 end )  as medium_fare2,
COUNT(case when fare_amount < 0 then 1 end ) as negative_fare2
